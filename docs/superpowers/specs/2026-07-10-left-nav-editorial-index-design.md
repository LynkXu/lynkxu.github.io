...
- Prefix the five primary destinations with tabular `01`–`05` indices.
...
- Up to 1100px: compose the navigation as a compact two-row masthead inside the existing single-column shell; do not change the shell breakpoint or content width.
...
- Automatically open the topics group when a secondary destination is active.
...
- A source-contract test locks the indices, topic auto-open behavior, search order, removal of the detached dot, and the mobile target size.
